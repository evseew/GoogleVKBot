...
chmod +x start_bot.sh stop_bot.sh control.sh restart.sh
...
# 7. Настройка crontab для обновления базы (УДАЛЕНО)
# echo "Настройка планировщика для обновления базы данных..." (УДАЛЕНО)
# (crontab -l 2>/dev/null; cat crontab_entry.txt) | crontab - (УДАЛЕНО)
...
# 9. Создание начальной базы данных (УДАЛЕНО, т.к. бот сам создаст/обновит при первом запуске)
# echo "Создание начальной базы данных..." (УДАЛЕНО)
# ./update_db.sh (УДАЛЕНО)
