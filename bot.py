    ...
    try:
        ...
        if is_from_manager:
            try:
                ...
                # await message.answer("🔇 Режим молчания бота активирован на 24 часа. Используйте слово 'speak' чтобы снова включить бота.")
...
# Добавляем обработчик команды для деактивации режима молчания
@router.message(F.text == "speak")
async def activate_bot_command(message: types.Message):
    """Деактивирует режим молчания и включает бота обратно в чате по слову 'speak'"""
    user_id = message.from_user.id
    chat_id = message.chat.id
    
    # Проверяем, что команду отправил менеджер или администратор
    if user_id in MANAGER_USER_IDS or user_id == ADMIN_USER_ID:
        # Проверяем, находится ли чат в режиме молчания
        if await is_chat_silent(chat_id):
            # Деактивируем молчание
            await set_chat_silence(chat_id, False)
            await message.answer("✅ Бот активирован и будет отвечать на сообщения в этом чате.")
            logging.info(f"Менеджер {user_id} вручную активировал бота в чате {chat_id}")
        else:
            await message.answer("ℹ️ Бот уже активен в этом чате.")
    else:
        # Если команду отправил не менеджер, игнорируем ее или отвечаем по желанию
        logging.info(f"Пользователь {user_id} без прав менеджера пытался активировать бота в чате {chat_id} словом 'speak'")
        # Можно отправить сообщение типа "У вас нет прав..." или просто проигнорировать
