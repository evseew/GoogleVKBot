    ...
    try:
        ...
        if is_from_manager:
            try:
                ...
                # await message.answer("🔇 Режим молчания бота активирован на 24 часа. Используйте /speak чтобы снова включить бота.")
...
async def activate_bot_command(message: types.Message):
    """Деактивирует режим молчания и включает бота обратно в чате"""
    user_id = message.from_user.id
    chat_id = message.chat.id
    ...
    if user_id in MANAGER_USER_IDS or user_id == ADMIN_USER_ID:
        ...
        logging.info(f"Пользователь {user_id} без прав менеджера пытался активировать бота в чате {chat_id}")
