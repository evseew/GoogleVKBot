        ...
        try:
            ...
            for event in current_longpoll.listen():
                if event.type == VkBotEventType.MESSAGE_NEW:
                    ...
                    if hasattr(event, 'obj'): # Убрана проверка is not None
                        logger.debug(f"Содержимое event.obj для пропущенного события: {event.obj}")
                    else:
                        logger.debug("Атрибут event.obj отсутствует.")
                    if hasattr(event, 'raw'): # Убрана проверка is not None
                        logger.debug(f"Содержимое event.raw для пропущенного события: {event.raw}")
                    else:
                        logger.debug("Атрибут event.raw отсутствует.")
