...
async def process_buffered_messages(peer_id: int, original_user_id: int):
    ...
    async with user_processing_locks[peer_id]:
        ...
        logger.info(f'{log_prefix} Объединенный запрос для peer_id={peer_id} ({num_messages} сообщ.): "{combined_input}"')
        ...
        for msg in messages_response.data:
            if msg.role == "assistant" and msg.run_id == run.id: # Ищем ответ именно этого run
                if msg.content and msg.content[0].type == 'text':
                    ...
                    logger.info(f"Получен ответ от ассистента для user_id={user_id}: {assistant_response}")
...
async def get_relevant_context(query: str, k: int) -> str:
    ...
    try:
        ...
        try:
            ...
            logger.debug(f"Эмбеддинг для запроса '{query}' создан.")
        except Exception as e:
        ...
        try:
            ...
            logger.debug(f"Поиск в ChromaDB для '{query}' выполнен.")
        except Exception as e:
        ...
        if not results or not results.get("ids") or not results["ids"][0]:
            logger.info(f"Релевантных документов не найдено для запроса: '{query}'")
            return ""
        ...
        context_pieces = []
        sources_used = set()
        logger.info(f"Найдено {len(documents)} док-в для '{query}'. Топ {k}:")
        for i, (doc, meta, dist) in enumerate(zip(documents, metadatas, distances)):
            source = meta.get('source', 'Неизвестный источник')
            logger.info(f"  #{i+1}: Источник='{source}', Дистанция={dist:.4f}, Контент='{doc}'")
            context_piece = f"Из документа '{source}':\n{doc}"
            context_pieces.append(context_piece)
            sources_used.add(source)

        if not context_pieces:
             logger.info(f"Не найдено подходящих фрагментов контекста для '{query}'.")
             return ""
...
async def handle_new_message(event: VkBotEvent):
    ...
    try:
        if event.from_user: # Обрабатываем только личные сообщения боту
            ...
            logger.debug(f"Полное событие VK [event.obj]: {event.obj}")
            if hasattr(event, 'raw'):
                logger.debug(f"Полное сырое событие VK [event.raw]: {event.raw}")
            ...
            if chat_silence_state.get(peer_id, False):
                logger.info(f"Бот в режиме молчания для peer_id={peer_id}. Сообщение '{message_text}' от user_id={user_id} игнорируется.")
                logger.debug(f"Игнорирование из-за режима молчания. peer_id={peer_id}, user_id={user_id}, text='{message_text}'")
                return # Игнорируем
            ...
            if last_time and now_dt - last_time < timedelta(seconds=MESSAGE_COOLDOWN_SECONDS):
                ...
                logger.debug(f"Игнорирование из-за кулдауна. peer_id={peer_id}, user_id={user_id}, text='{message_text}', last_time={last_time}, now_dt={now_dt}")
            ...
            logger.info(f"Получено сообщение от user_id={user_id} (peer_id={peer_id}): '{message_text}'")
            ...
            user_id = event.obj.message['from_id']
            message_text = event.obj.message['text'].strip()
            logger.debug(f"Получено сообщение в чате {chat_id} от {user_id}: {message_text}")
